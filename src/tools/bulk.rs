...
pub(super) fn tools() -> Vec<Tool> {
    ...
    vec![
        ...
        tool_with_annotations("leantime_bulk_update_tickets", format!("Update multiple tickets in one call (max {}). Uses the safe patch API — only provided fields change, others are never wiped. Results are per-item: some may succeed while others fail. ALWAYS call with dryRun: true first — validate the whole batch and show the per-item preview, then execute on approval.{}", MAX_BATCH, rate),
