...
pub(super) fn tools() -> Vec<Tool> {
    ...
    vec![
        ...
        tool_with_annotations("leantime_update_milestone", format!("Update an existing milestone. Only the provided fields are changed (Leantime's patch API — other fields are never wiped). The description is {}. Call directly when every value was explicitly given or resolves unambiguously. Prefer dryRun: true first when you interpreted the request or chose values yourself — show the from → to diff and ask for confirmation before executing.", md),
